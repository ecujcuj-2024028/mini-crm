...
watch(searchInput, (newVal) => {
  ...
  searchTimer = setTimeout(() => {
    userStore.searchQuery = newVal.trim();
    userStore.roleFilter = selectedRole.value || null;
    userStore.fetchUsers(1);
  }, 300);
});
...
const handleSearch = () => {
  if (searchTimer) clearTimeout(searchTimer);
  userStore.searchQuery = searchInput.value.trim();
  userStore.roleFilter = selectedRole.value || null;
  userStore.fetchUsers(1);
};

// Limpiar todos los filtros y restablecer busqueda
const clearFilters = () => {
  if (searchTimer) clearTimeout(searchTimer);
  searchInput.value = '';
  selectedRole.value = '';
  userStore.searchQuery = '';
  userStore.roleFilter = null;
  userStore.fetchUsers(1);
};
...
// Abrir modal de confirmacion elegante para desactivar/restaurar
const promptToggleUser = (user) => {
  userToToggle.value = user;
  showConfirmModal.value = true;
};

// Confirmar desactivar o restaurar
const handleConfirmToggle = async () => {
  ...
  try {
    if (user.isActive) {
      ...
      toastStore.addToast({ title: 'Usuario Desactivado', message: `${user.name} ha sido desactivado correctamente.`, type: 'info' });
    } else {
      await userStore.restoreUser(user.id);
      toastStore.addToast({ title: 'Usuario Restaurado', message: `${user.name} fue restaurado exitosamente.`, type: 'info' });
    }
  } catch (err) {
...
const handleSaveUser = async (payload) => {
  try {
    if (selectedUser.value) {
      ...
      toastStore.addToast({ title: 'Usuario Actualizado', message: 'Los datos del usuario fueron guardados correctamente.', type: 'info' });
    } else {
      await userStore.createUser(payload);
      toastStore.addToast({ title: 'Usuario Creado', message: 'El nuevo usuario ha sido registrado exitosamente.', type: 'info' });
    }
...
<template>
  <div class="space-y-6">
    <!-- Subtitulo Descriptivo y Boton Nuevo Usuario (Sin duplicar el titulo principal de AppHeader) -->
    ...
    <div class="bg-white rounded-3xl shadow-sm border border-[#E4EAED] overflow-hidden">
      <div class="overflow-x-auto">
        <table class="w-full text-left border-collapse">
          <thead>
            <tr class="bg-[#F3F6F7] border-b border-[#E4EAED] text-[11px] font-mono uppercase tracking-wider text-[#6E6E6E]">
          ...
          <tbody class="divide-y divide-[#E4EAED] text-sm">
            <tr v-if="userStore.loading" class="text-center py-8">
              <td colspan="4" class="p-8 text-[#6E6E6E]">Cargando usuarios...</td>
            </tr>
            <tr v-else-if="userStore.users.length === 0" class="text-center py-8">
              <td colspan="4" class="p-8 text-[#6E6E6E]">No se encontraron usuarios.</td>
            </tr>
            <tr
              v-else
              v-for="user in userStore.users"
              :key="user.id"
              :class="['hover:bg-[#F3F6F7]/50 transition-colors', !user.isActive ? 'opacity-60 bg-gray-50' : '']"
            >
              ...
              <td class="py-4 px-6">
                <div class="flex items-center space-x-3">
                  ...
                  <div>
                    <p class="font-bold text-[#263840] leading-tight">
                      {{ user.name }}
                    </p>
              ...
              <!-- Columna Rol -->
              <td class="py-4 px-6 font-medium text-[#4A4A4A]">
              ...
              <td class="py-4 px-6 text-right">
                <div class="flex items-center justify-end space-x-2">
                  ...
                  <button
                    ...
                    class="w-8 h-8 rounded-lg bg-[#F3F6F7] hover:bg-[#E4EAED] text-[#5C7E8F] flex items-center justify-center transition-colors cursor-pointer"
                  >
                    <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  ...
                  <button
                    ...
                    class="w-8 h-8 rounded-lg bg-[#F3F6F7] hover:bg-[#E4EAED] text-[#263840] flex items-center justify-center transition-colors cursor-pointer"
                  >
                    <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  ...
                  <!-- Boton Desactivar / Restaurar (Abre Modal Elegante) -->
                  <button
      ...
      <div class="border-t border-[#E4EAED] px-4">
        <AppPagination
          :page="userStore.page"
          :totalPages="userStore.totalPages"
          :totalItems="userStore.total"
          :itemsPerPage="userStore.limit"
    ...
    <AppConfirmModal
      ...
      :message="userToToggle?.isActive
        ? `¿Estás seguro de que deseas desactivar a ${userToToggle?.name} (${userToToggle?.email})? El usuario no podrá iniciar sesión.`
        : `¿Deseas restaurar el acceso para ${userToToggle?.name} (${userToToggle?.email})?`"
    ...
    <!-- Modales para Crear/Editar y Resetear Clave -->
    <UserModal
      :show="showUserModal"
      :user="selectedUser"
      :loading="userStore.loading"
